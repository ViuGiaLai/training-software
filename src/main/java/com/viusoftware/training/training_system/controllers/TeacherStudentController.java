...
public class TeacherStudentController {
    ...
    @GetMapping("/gv/hoc-sinh-cua-toi")
    public String myStudents(Model model) {
        ...
        List<UsersStudents> students = java.util.Collections.emptyList();
        if (teacher != null) {
            // Sử dụng fetch join để lấy luôn students, tránh lazy loading lỗi
            classRoom = classRoomRepository.findAllWithStudents().stream()
        ...
        model.addAttribute("contentFragment", "gv/hoc-sinh-cua-toi");
        return "gv/teacher";
    }

    @Transactional(readOnly = true)
    @GetMapping("/gv/hoc-sinh-cua-toi/fragment")
    public String myStudentsFragment(Model model) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String username = auth != null ? auth.getName() : null;
        UsersTeachers teacher = usersTeachersRepository.findByUsername(username);
        ...
        model.addAttribute("classRoom", classRoom);
        model.addAttribute("students", students);
        return "gv/hoc-sinh-cua-toi :: content";
    }
}
